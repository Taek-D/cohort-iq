...
### 통계적 한계
- ~~유의성 검정 미적용~~ → **v2.0에서 해결**: Chi-Square 독립성 검정, Kaplan-Meier 생존 분석, Log-Rank Test를 웹앱 내 순수 JavaScript로 구현 (외부 라이브러리 없이 불완전 감마함수까지 자체 구현)
- **신뢰구간 미표시**: 코호트 크기가 작을 경우 리텐션율의 변동성이 크지만, 이를 반영하지 않음
- **인과관계 미추론**: 리텐션 차이의 원인(마케팅 채널, 시즌, 제품 변경 등)을 식별하지 않음. A/B 테스트 없이는 확정 불가

### 데이터 한계
- ~~매출 데이터 미연동: LTV 추정 불가~~ → **v2.0에서 해결**: 지수 감쇠 모델(λ = -ln(R_last/R_prev)/Δt)을 통한 코호트별 LTV 예측 구현. 다만 실제 매출 데이터가 아닌 리텐션 기반 추정이므로, ARPU 가정에 의존
...
| 개선 항목 | 설명 | 우선순위 | 상태 |
|-----------|------|----------|------|
| **BG/NBD 모델** | 확률적 이탈 예측 (Fader & Hardie, 2005). 각 사용자의 "살아있을 확률"을 산출 | P0 | 미구현 |
| **Gamma-Gamma 모델** | 실제 매출 데이터 연동 시 기대 LTV 추정 | P1 | 미구현 |
| ~~코호트 간 통계 검정~~ | ~~카이제곱 검정으로 리텐션 차이 유의성 판별~~ | ~~P1~~ | **✅ 완료** (v2.0) |
| ~~LTV 예측~~ | ~~리텐션 기반 고객 생애 가치 추정~~ | ~~P1~~ | **✅ 완료** (v2.0) |
| **Monthly 코호트 옵션** | 주간 외 월간 코호트 지원 | P2 | 미구현 |
| **이벤트 유형 필터** | 핵심 행동만 선별하여 "진정한 리텐션" 측정 | P2 | 미구현 |
| **신뢰구간 표시** | 코호트 크기 기반 Wilson score interval | P2 | 미구현 |
| **A/B 테스트 시뮬레이션** | 온보딩 변경 효과를 사전 시뮬레이션 | P2 | 미구현 |

---

## 6. 통계적 검증 방법론

본 프로젝트에서 수행하는 통계 검정입니다. **웹앱(`statisticalTests.js`)에서 순수 JavaScript로 실시간 수행**되며, Python 노트북(`analysis/cohort_eda.ipynb`)에서도 `scipy`/`lifelines`로 교차 검증됩니다.
